...
from teledesk.config.settings import AppSettings, SettingsStore, bootstrap_environment
from teledesk.config.version import current_version
...
from teledesk.services.portable_updater import PortableUpdater
...
def _maybe_prompt_for_updates(
    *,
    ...
    from teledesk.ui.dialogs.update_dialog import UpdateDialog
    ...
    settings_store.save(settings)
    wx.MessageBox(
        "The update has been staged. Teledesk will now exit so the portable files can be replaced.",
        "Applying update",
        wx.OK | wx.ICON_INFORMATION,
    )
    raise SystemExit(0)
